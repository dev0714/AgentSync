-- Minting an installation token needs the App ID, not the slug.
--
-- The slug is cosmetic — it is what appears in github.com/apps/<slug>. The JWT
-- that exchanges the private key for an installation token is issued against
-- the numeric App ID, so storing only the slug would have left the connection
-- unusable by the worker that is meant to consume it.
alter table agentsync.github_app_installations
  add column if not exists app_id bigint;

drop function if exists agentsync.connect_github(uuid, text, text, bigint, text, text, text[], integer, boolean);
...
begin
  ...
  if p_app_id = p_installation_id then
    -- They are different numbers on the same page and are easy to swap; a
    -- match is far more likely a mistake than a coincidence.
