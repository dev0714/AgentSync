-- Hold the GitHub connection to the same rule as the deployment one.
--
-- connect_github() only rejected a pasted PEM, which catches a private key and
-- nothing else. Requiring a scheme — env:NAME, sm://path — makes "this is a
-- reference, not a secret" something the database can check rather than a
-- convention the form politely asks for.
...
begin
  ...
  if not agentsync.is_secret_reference(trim(p_private_key_reference)) then
  ...
  if v_hook is not null and not agentsync.is_secret_reference(v_hook) then
    return jsonb_build_object('ok', false, 'error', 'SECRET_VALUE_NOT_A_REFERENCE');
  end if;
